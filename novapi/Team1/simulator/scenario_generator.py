...
def SelectCube(k):
    global cube,cube_select
    cube[cube_select] = f"{cube[cube_select].replace(' <', '')}"
    cube[cube_select] = f"{cube[cube_select]} <"
...
while True:
    ...
    cube_select = cube_select + 1 if keys == "d" else cube_select - 1 if keys == "a" else 0
    SelectCube(keys)
